...
if __name__ == "__main__":
    ...
    agent = Agent(obs_dim=obs_dim + args.num_agents, act_dim=act_dim).to(device)
    optimizer = optim.Adam(agent.parameters(), lr=args.learning_rate, eps=1e-5)

    # Storage setup
    obs = torch.zeros((args.num_steps, args.num_envs * args.num_agents, obs_dim + args.num_agents)).to(device)
    ...
    for update in range(1, num_updates + 1):
        ...
        # flatten the batch
        b_obs = obs.reshape((-1, obs_dim + args.num_agents))
